{
  services.transmission = { 
    ...
    settings = { #Override default settings
      ...
      peer-limit-per-torrent = 48;
      max-peers-global = 10000; #old version
      peer-limit-global = 10000;
      download-queue-enabled = false; #Boolean (default = true) When true, Transmission will only download download-queue-size non-stalled torrents at once.
      download-queue-size = 0; #Number (default = 5) See download-queue-enabled
      seed-queue-enabled = false;
      seed-queue-size = 0; #Number (default = 10) See seed-queue-enabled.
      cache-size-mb = 4096; #cache-size-mb: Number (default = 4), in megabytes, to allocate for Transmission's memory cache. The cache is used to help batch disk IO together, so increasing the cache size can be used to reduce the number of disk reads and writes. The value is the total available to the Transmission instance. Setting this to 0 bypasses the cache, which may be useful if your filesystem already has a cache layer that aggregates transactions.
